...
hl.monitor({
    ...
    mode     = "preferred",
    position = "auto",
    scale    = 1,   -- integer scale: crisp in every app, no fractional downscaling (auto picked 1.5)
})
...
hl.on("hyprland.start", function ()
    ...
    hl.exec_cmd("gsettings set org.gnome.desktop.interface cursor-size 28 && gsettings set org.gnome.desktop.interface font-name 'Inter 11' && gsettings set org.gnome.desktop.interface monospace-font-name 'Geist Mono 11'")
    -- Mousepad as a TextEdit equivalent: Helvetica clone (Nimbus Sans) like TextEdit's default
    hl.exec_cmd("gsettings set org.xfce.mousepad.preferences.view use-default-monospace-font false && gsettings set org.xfce.mousepad.preferences.view font-name 'Nimbus Sans 10'")
end)
...
-- macOS-plain (icons stow package) inherits it with black hands
-- macOS-accent -> macOS-accent-dark/light (generated by scripts/theme-toggle.sh) inherits macOS-plain, adds a thin Windows-style I-beam and accent spinner (white arrow/hands in light mode)
hl.env("XCURSOR_THEME", "macOS-accent")
-- size 24 draws the 17x23 macOS arrow at 1x; 28 is the closest to its physical size on this ~143 ppi panel
